...
def prepare_patches_src(img_gray, indices_pixel, window_size=11, window_step=2):
    ...
    # print(indices_pixel.device, kernel_patch.device)
    # exit("hello here")
    if indices_pixel.device != kernel_patch.device:
        print("they are different now")
        # print(indices_pixel.device,kernel_patch.device)
        indices_pixel = indices_pixel.to(kernel_patch.device)
    indices_patch = indices_pixel.reshape(num_pixels, 1, 1, 2) + kernel_patch

    # sample img
    mask_indices_inside = is_inside_border(indices_patch.reshape(-1, 2), size_img).reshape(
        indices_patch.shape[:3])  # N*M*M
    indices_patch_inside = (indices_patch * mask_indices_inside[..., None]).reshape(-1,
                                                                                    2)  # N*2. let the indices of outside pixels be zero

    if img_gray.ndim == 2:
        rgb_patches = img_gray[indices_patch_inside[:, 0], indices_patch_inside[:, 1]].reshape(
            indices_patch.shape[:3])  # N*M*M
    elif img_gray.ndim == 3:
        rgb_patches = img_gray[indices_patch_inside[:, 0], indices_patch_inside[:, 1]].reshape(
            indices_patch.shape[:3] + tuple([3]))  # N*M*M
    else:
...
def sample_patches(img, idx_patches_input, sampling_mode='nearest'):
    ...
    mask_indices_inside = is_inside_border(idx_patches.reshape(-1, 2), size_img).reshape(
        idx_patches.shape[:-1])  # N*M*M
    indices_patch_inside = (idx_patches * mask_indices_inside[..., None]).reshape(-1,
                                                                                  2)  # N*2. let the indices of outside pixels be zero
    ...
    if sampling_mode == 'grid_sample':
        ...
        rgb_patches = F.grid_sample(img[None, None, :, :], idx_patches_input_norm.reshape(1, 1, -1, 2),
                                    align_corners=False).squeeze()
        rgb_patches = rgb_patches.reshape(shape_patches[:3])
    elif sampling_mode == 'nearest':
        if img.ndim == 2:
            rgb_patches = img[indices_patch_inside[:, 0], indices_patch_inside[:, 1]].reshape(
                idx_patches.shape[:-1])  # N*M*M
        elif img.ndim == 3:
            rgb_patches = img[indices_patch_inside[:, 0], indices_patch_inside[:, 1]].reshape(
                idx_patches.shape[:-1] + tuple([3]))  # N*M*M
        else:
            raise NotImplementedError
...
def compute_NCC_score(patches_ref, patches_src):
    ...
    calculate_patches_mean = lambda patches, mask: (patches * mask).reshape(patches.shape[0], -1).sum(axis=1) / (
            mask.reshape(patches.shape[0], -1).sum(axis=1) + 1e-6)
...
def visualize_sampled_pixels(img_input, indices_pixel, path=None):
    ...
    for i in range(indices_pixel.shape[0]):
        cv2.circle(img, (indices_pixel[i, 1], indices_pixel[i, 0]),
                   ...
                   thickness=2)
        cv2.putText(img, f'{i}', (
            indices_pixel[i, 1] + 2 if indices_pixel[i, 1] + 2 < img.shape[1] else img.shape[1] - 1,
            indices_pixel[i, 0]),
    ...
    for i in range(patch_num_row):
        for j in range(patch_num_row):
            ...
            if idx_patch >= num_patches:
                continue
            img[i * (w_p + interval):(i + 1) * (w_p + interval) - interval,
            j * (h_p + interval):(j + 1) * (h_p + interval) - interval] = patches[idx_patch]
    cv2.imwrite(path, img)
    return img
...
def compute_homography(pt_ref, n_ref, K, extrin_ref, extrin_src):
    ...
    mat_homography = (K[None, ...] @ (
            R_rel[None, ...] + t_rel[None, ...] @ n_ref.reshape(num_pts, 1, 3) / d)) @ torch.linalg.inv(K)
...
def warp_patches(idx_patches_xy, homography):
    ...
    if idx_patches_xy.ndim == 2:  # N*2
        ...
        idx_patches_warp = (homography.reshape(num_patches, 1, 1, H_patch, W_patch) @ idx_patches_homo[
            ..., None]).squeeze()  # N*M*M*3*1
    else:
        raise NotImplementedError
